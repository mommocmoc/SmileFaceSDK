...
namespace FaceControlSDK
{
    ...
    public class WebCamTextureToMatHelperSDK : MonoBehaviour
    {
        /// <summary>
        /// The mat Alpha value.
        /// </summary>
        public int matAlpha = 255;
        /// <summary>
        ...
        int _webCamListIndex = 0;
        //B키를 누르면 카메라 인풋 소스를 바꿀 수 있습니다.
        protected virtual void changeDevice()
        {
            ...
            if (Input.GetKeyDown(KeyCode.B))
            {
                ...
                if (_webCamListIndex >= devices.Length)
                {
                    _webCamListIndex = 0;
                }
                
                Stop();
                webCamDevice = devices[_webCamListIndex];
                hasInitDone = false;
                Debug.Log(webCamDevice.name);
                webCamTexture = new WebCamTexture(webCamDevice.name, webCamTexture.width, webCamTexture.height,(int)requestedFPS);
                Initialize();
            }
        }
        ...
        //Camera UI를 보이기/숨기기를 토글할 수 있습니다.
        //"C"를 누르면 Inspector에서 제어하지 않고 바로 토글할 수 있습니다.
        public bool renderSwitch = true;
        protected virtual void onOffCameraRederer()
        {
            if (Input.GetKeyDown(KeyCode.C))
            {
                renderSwitch = !renderSwitch;
            }
            this.gameObject.GetComponent<MeshRenderer>().enabled = renderSwitch;
        }
            ...
            if (hasInitDone)
            {
                ...
                if (screenOrientation != Screen.orientation && (screenWidth != Screen.width || screenHeight != Screen.height))
                {
                  //  changeDevice();
                    ...
                    frameMat = new Mat(webCamTexture.height, webCamTexture.width, CvType.CV_8UC4, new Scalar(0, 0, 0, matAlpha));
        ...
        /// </summary>
        ///
        private bool checkFrameUpdate = true;
        int cameraChnegeIndex = 0;
        protected virtual IEnumerator _Initialize()
        {
            ...
            // while(!checkFrameUpdate)
            // {
            //     Debug.Log("Cam Chnage Mode On");
            //     if (Input.GetKeyDown(KeyCode.B))
            //     {
            //         cameraChnegeIndex++;
            //     }
            //     if (cameraChnegeIndex >= devices.Length)
            //     {
            //         cameraChnegeIndex = 0;
            //     }
            //     if (hasInitDone)
            ...
            //     isInitWaiting = true;
            //     
            //     webCamDevice = devices[cameraChnegeIndex];
            //     webCamTexture = new WebCamTexture(webCamDevice.name, requestedWidth, requestedHeight, (int)requestedFPS);
            //     webCamTexture.Play();
            //     checkFrameUpdate = true;
            // }
